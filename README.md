...
| `/sell <单价> [个人限制。可选，留空或者-1表示无限制] [总库存。可选，留空或者-1表示无限制] [重置时间h。可选，个人限制重置周期，单位小时，0或留空表示不重置，默认0]` | OP 专用，将手持物品上架到官方商城出售区（记录完整 NBT，包括数量所以建议手持 1 个） |

| `/recycle <单价> [个人限制。可选，留空或者-1表示无限制] [总回收量。可选，留空或者-1表示无限制] [重置时间h。可选，个人限制重置周期，单位小时，0或留空表示不重置，默认0]` | OP 专用，将手持物品添加到官方商城回收区（记录完整 NBT，包括数量所以建议手持 1 个） |
...
auction:
  ...
  tax_rate: 0.02                 # 2% 拍卖税率 (成交后从卖家所得中扣除)

  max_items_per_page: 30         # 每页显示数量
...
update_checker:
  ...
  version_check_url: "https://gist.githubusercontent.com/intellectmind/325d2817045df2f44882de01fd2057e6/raw/WebMarket"
