...
| `/wm sj <数量> <单价>` | 快速上架手中物品（玩家使用，可批量上架） |
| `/wm sg <数量> <单价>` | 快速收购手中物品（玩家使用） |
| `/wm pm <数量> <起拍价> <时间(1-48小时)> [一口价]` | 快速拍卖手中物品（玩家使用，可批量拍卖） |
| `/wm sell <单价> [个人限制] [总库存] [个人限制重置周期h]` | OP 专用，将手持物品上架到官方商城出售区（记录完整 NBT，包括数量所以建议手持 1 个）。个人限制，留空或者-1表示无限制。总库存，留空或者-1表示无限制。个人限制重置周期，单位小时，0或留空表示不重置 |
| `/wm recycle <单价> [个人限制] [总回收量] [个人限制重置周期h]` | OP 专用，将手持物品添加到官方商城回收区（记录完整 NBT，包括数量所以建议手持 1 个）。个人限制，留空或者-1表示无限制。总回收量，留空或者-1表示无限制。个人限制重置周期，单位小时，0或留空表示不重置 |
| `/wm buff <buff类型> <等级> <每秒价格> [单次最少秒数] [单次最多秒数] [个人限购秒数] [总库存秒数] [重置时间h]` | OP 专用，添加物品到官方商城Buff区。可在游戏内输入/buff查看详细帮助 |
| `/wm specialitem <名称> <描述> <价格> <图标材质> <指令1 指令2 ...> [个人限购] [总库存] [重置时间h]` | OP 专用，添加物品到官方商城特殊物品区。支持变量{player}，多条指令以竖线相隔。可在游戏内输入/specialitem查看详细帮助 |
...
- `webmarket.tax.auction.<税率>`

  拍卖手续费率（小数）

提示：将权限节点中的 `<数值>`/`<天数>`/`<税率>` 替换为具体数字即可生效。
...
1. 插件使用需激活码，每个激活码同时仅限一个Minecraft服务器(1个IP)使用。

2. 如更换IP后显示激活失败，请等待40分钟后再次尝试。
